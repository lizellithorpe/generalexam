...
%with close approaches to have prohibitively small time steps, the idea of a hybrid symplectic integrator came about. 
 \\
\indent A change came to the \textit{Mercury} code as authors \citep{cha02} then turned to the next problem: the inclusion of additional stellar-mass bodies, such as a 
binary companion, that interact with the planetary system. \cite{cha02} explains the inclusion of a hierarchical coordinate system
...
to allow the inclusion of multiple bound massive bodies (i.e., a binary or triplet star system) and unbound massive bodies (i.e. stars in a cluster environment). The details 
of the changes to the integrator are explained in \citet{kai17}.
...
are also integrated symplectically, although with a mixed variable symplectic Wisdom-Holman scheme \citep{wis91}. Finally, when interacting bodies
near each other and as such
non-Keplerian terms in the Hamiltonian become
comparatively large, the bodies are integrated directly with a Bulirsch-Stoer scheme. For the planets, this occurs when passing 
within a hill radii of one another. For cluster stars with much larger hill radii, the changeover to a direct integration
occurs when the time-scale of the close encounter is less than $500\times$ the simulation time-step of $100$ days.
%The importance of these terms is determined by a changeover factor $K(r_{ij})$, 
%evaluated based on the separation between two bodies $i$ and $j$, in the Hamiltonian. 
%The Hamiltonian splitting in \citet{cha99}, in inertial coordinates, uses such a changeover function (Eqn. \ref{ham}).
%\begin{equation}
%\begin{split}
    %H_A = \sum_{i=1}^N\left(\frac{p_i^2}{2m_i}-\frac{Gm_{sun}m_i}{r_{sun}} \right) - G \sum_{i=1}^N\sum_{j=1}^N\frac{m_im_j}{r_ij}(1-K(r_{ij})) \\
    %H_B = -G\sum_{i=1}^N\sum_{j=i+1}^N\frac{m_im_j}{r_ij}K(r_{ij})
%\end{split} 
%\label{ham}
%\end{equation}
Thus, this scheme largely preserves the angular momentum and energy conservation of a symplectic scheme while allowing
...
namely the planets, the binary, and the cluster star. We begin with the expression for the mass distribution of a Plummer sphere in inertial coordinates, where
$\vec{x}$ is defined relative to the clusters's center-of-mass. 
\begin{equation}
\rho (\vec{x}) = \frac{3M_0}{4\pi a_0} \times \biggl(1+\frac{|\vec{x}|^2}{a_0^2} \biggr)^{-5/2}
\end{equation}
...
%As these simulations are run in democratic heliocentric coordinates, all other stars are redefined with respect to the rest frame of the primary. 
...
\begin{figure}
    \plotone{fig/lost_planets_suite17_sim_51.png}
...
%that 1 or more of the system's planets will also be lost. 
...
The main takeaway from this project is that the orbital architectures of surviving planets in unstable (lose at least one planet)
 ...
 unstable systems are statistically distinct from their stable counterparts. High eccentricity planets, like the ones we 
 see in our unstable systems, are thought to have a dynamical source. First posited
 in \citet{ras96}, planet-planet scattering interactions can lead to ejections of some planets and a higher eccentricity for planets that remain.
  %and later expanded on in such works like \citet{dlfm97} and \citet{for08}.
  In our systems, often these surviving planets are the two inner gas giants. The Jupiter and Saturn mass planets 
  thus act as good markers of a past instability, and our analysis will focus on their final orbits in each simulation.
The distributions of the final eccentricity and inclination of the inner giants are shown in Figure \ref{fig:160_ecc_inc}, 
 ...
 and $p_{KS} = 1.5\times 10^{-26}$ for the inclinations distributions) shows that we can reasonably reject the null hypothesis that these two sets are drawn 
...
momentum. If the planets stay bound, this interaction can heat up their orbits to higher eccentricities and inclinations. Oftenthe two
...
high predicted spin-orbit angles. Noting also that the unstable systems are also likely to be very eccentric, their orbits could become 
circularized by tidal forces, leading to a more closely orbiting hot Jupiter with a misaligned spin-orbit angle. As our simulations 
do not include tidal forces, however, the further evolution of these unstable systems remains a mystery.
